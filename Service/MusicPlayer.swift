...
final class MusicPlayer: NSObject {
    ...
    func stopBackgroundMusic(completion: (() -> Void)? = nil) {
        fadeOutTimer?.invalidate() // Ensure to invalidate any existing timer before creating a new one
        fadeOutTimer = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { [weak self] _ in
    ...
    private func fadeOutVolume(completion: (() -> Void)? = nil) {
        ...
        // New fadeOut implementation
        let fadeOutSteps = 6 // Total steps based on interval and duration
        let fadeStep = 1.0 / Float(fadeOutSteps) // Adjust this based on actual fade duration and steps
...
extension MusicPlayer: AVAudioPlayerDelegate {
    func audioPlayerDidFinishPlaying(_ player: AVAudioPlayer, successfully flag: Bool) {
        if flag {
            completionHandler?() // Call the completion handler if the audio finished successfully
        }
    }
}
